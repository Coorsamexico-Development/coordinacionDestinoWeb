...
const props = defineProps({
    ...
    incidencias: Object,
    oc: Object, // This will be either the OC or Factura object
    dt: Number,
    type: {
        type: String,
...
axios
    .get("/getTiposIncidenciaYProductos")
    .then((response) => {
...
const eliminarIncidencia = (
    incidencia, //funcion para eliminar las evidencias de la bd
) => {
    //console.log(incidencia)
    axios
        .post(
            route("borrarIncidencia", {
                incidencia_id: incidencia,
            }),
        )
        .then((response) => {
            emit("reconsultarIncidencias", props.dt, props.oc.id, props.type);
        })
        .catch((err) => {
            console.log(err);
        });
...
const saveNewIncidencias = () => {
    const routeName =
        props.type === "oc"
            ? "saveIncidenciasByOc"
            : "saveIncidenciasByFactura";
    const params = {
        incidencias: newInidencias.value,
        confirmacion: props.dt,
    };

    if (props.type === "oc") {
        params.oc = props.oc.id;
    } else {
        params.factura_id = props.oc.id;
    }
...
const eliminarNuevaIncidencia = (key) => {
    //console.log(key)
    let incidenciasNuevas = newInidencias.value.filter(
        (newIncidencia, index) => {
            index !== key;
        },
    );
    ...
    <DialogModal :maxWidth="tamañoModal" :show="show" @close="close()">
        ...
        <template #content>
            <table class="w-full" style="font-family: &quot;Montserrat&quot;">
                <thead>
                    <tr class="border-b">
                        ...
                        <td class="font-semibold text-center">Factura</td>
                ...
                <tbody>
                    <tr v-for="incidencia in incidencias" :key="incidencia.id">
                        <td class="py-2">
                            <button
                                v-if="
                                    $page.props.auth.user.cans[
                                        'delete-incidencia-customer'
                                    ]
                                "
                                @click="eliminarIncidencia(incidencia.id)"
                                class="px-1 py-1 bg-red-500 rounded-lg"
                            >
                                <img
                                    class="w-3 h-4"
                        ...
                        <td class="flex items-center justify-center py-2">
                            <div v-if="incidencia.evidencias.length > 0">
                                <div
                                    v-for="(
                                        evidencia, key
                                    ) in incidencia.evidencias"
                                    :key="evidencia.id"
                                >
                                    <div>
                                        <a
                                            v-if="key == 0"
                                            :href="evidencia.evidencia"
                    ...
                    <tr
                        v-for="(newIncidencia, key) in newInidencias"
                        :key="key"
                    >
                        <td class="py-2 text-center">
                            <button
                                ...
                                "
                                @click="eliminarNuevaIncidencia(key)"
                                class="px-1 py-1 bg-red-500 rounded-lg"
                            >
                                <img
                                    class="w-3 h-4"
                        ...
                        <td class="py-2 text-center">
                            <SelectComponent
                                class="w-30"
                                v-model="newIncidencia.tipo_incidencia_id"
                            >
                                <option
                                    v-for="tipo_incidencia in tipo_incidencias"
                                    :value="tipo_incidencia.id"
                                    :key="tipo_incidencia.id"
                                >
                        ...
                        <td class="py-2 text-center">
                            <TextInput
                                class="w-14"
                                v-model="newIncidencia.cantidad"
                            />
                        </td>
            ...
            <div
                ...
                v-if="newInidencias.length !== 0"
            >
                <button
                    @click="saveNewIncidencias()"
                    class="text-white bg-[#44BFFC] px-10 py-4 rounded-full"
                >
                    Guardar
                </button>
            </div>
            <div class="flex justify-center mt-2">
                <button
                    v-if="
                        $page.props.auth.user.cans[
                            'create-new-incidencia-customer'
                        ]
                    "
                    @click="addNewIncidencia()"
                    class="p-1 px-3 border-2 rounded-full"
                >
