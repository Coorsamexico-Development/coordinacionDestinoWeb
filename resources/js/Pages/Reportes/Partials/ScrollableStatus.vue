...
const contadorIndividual = computed(() => {
    return props.contadores.filter(
        (contador) => contador.status_padre == props.statu.id,
    );
});
</script>
<template>
    <div :id="'status-' + statu.id">
        <div class="flex flex-row justify-between my-2">
            <h1
                ...
                :id="'status-name-' + statu.id"
            >
                {{ statu.nombre }}
            </h1>
            <div v-if="statu.id == 1">
                <a
                    :href="route('downloadReport')"
                    id="downloadReport"
                    class="px-2 rounded-xl bg-[#697FEA] py-1"
                >
                    <span
                        style="font-family: &quot;Montserrat&quot;"
                        class="text-white"
                        >Descargar ejemplo</span
                    >
                </a>
            </div>
            <div v-if="statu.id == 1">
            ...
            <table>
                <thead>
                    <tr>
                        <th></th>
                        <th
                            :id="'cotador-' + contador.id"
                            scape="col"
                            class="p-4"
                            v-for="contador in contadorIndividual"
                            :key="contador.id"
                            :style="{ backgroundColor: contador.color }"
                        >
