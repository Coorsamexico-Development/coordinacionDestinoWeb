...
const showClients = (
    ubicacion_id, //funcion para desplegar
) => {
...
const consultarDts = () => {
    if (params.ubicacion_id !== -1) {
        axios
            ...
            .catch((e) => {
                console.error("UbicacionDeplegable2: Error fetching data", e);
            });
    }
};
...
const contador = computed(() => {
    ...
    for (
        let index = 0;
        index < props.ubicacion.confirmaciones_dts.length;
        index++
    ) {
        const confirmacion = props.ubicacion.confirmaciones_dts[index];
        if (
            confirmacion.ubicacion_id == props.ubicacion.id &&
            confirmacion.status_id == props.status.id
        ) {
...
<template>
    <div class="pb-1 bg-white rounded-xl drop-shadow-lg">
        <!--main-->
        <div>
            <!--Header-->
            <div
                class="flex flex-row items-center justify-between p-4 mx-2 mt-4 bg-white rounded-lg"
            >
                <h1 class="text-lg uppercase" style="font-family: Montserrat">
                    {{ ubicacion.nombre_ubicacion }}
                </h1>
                <div class="flex flex-row items-center">
                    <div>
                        <h1
                            class="mx-2 text-3xl font-bold"
                    ...
                    <div>
                        ...
                        <svg
                            @click="showClients(ubicacion.id)"
                            v-if="!show"
                    ...
                    <div v-for="dt in dtsData" :key="dt.id">
                        <DtBlock
                            ...
                            @deleted="
                                () => {
                                    console.log(
                                        'UbicacionDeplegable2: Event @deleted received',
                                    );
                                    consultarDts();
                                }
                            "
